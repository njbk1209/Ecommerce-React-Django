...
#Optiene los Items dentro de Cart
class GetItemsView(APIView):
    def get(self, request, format=None):
        ...
        try:
            ...
            #Si existe cartItem en Cart
            if CartItem.objects.filter(cart=cart).exists():

                #Agrega uno por uno el cartItem a result
                for cart_item in cart_items:
                    ...
                    result.append(item)
            #Retorna satisfactoriamente los items        
...
#Agregar Item al carrito a traves de un POST
class AddItemView(APIView):
    def post(self, request, format=None):
        ...
        try:
            #Si el producto no existe no se agrega y arroja error
            if not Product.objects.filter(id=product_id).exists():
                return Response(
                    {'error': 'This product does not exist'},
                    status=status.HTTP_404_NOT_FOUND)
            
            #Recibe producto y carrito de usuario
            product = Product.objects.get(id=product_id)
            cart = Cart.objects.get(user=user)

            #Si el item ya esta en carrito, no se puede agregar al carrito nuevamente.
            if CartItem.objects.filter(cart=cart, product=product).exists():
                return Response(
                    {'error': 'Item is already in cart'},
                    status=status.HTTP_409_CONFLICT)

            #Si no esta se evalua la cantidad
            if int(product.quantity) > 0:
                CartItem.objects.create(
                    product=product, cart=cart, count=count
                )
                
                #Si existe el item y actualizamos la cantidad se suma.
                if CartItem.objects.filter(cart=cart, product=product).exists():
                    ...
                    #Determinar la cantidad por cartItem
                    for cart_item in cart_items:
                    ...
                    #Retorna exitosamente el carrito con los items
...
class GetTotalView(APIView):
    def get(self, request, format=None):
        ...
        try:
            ...
            #Si Existen cartItems se calcula el precio total de todo el carrito
...
#Total del Item cart, basado en cantidades y precios
...
#Actualizacion de datos de los items dentro del carrito
...
#Remover itemsCart dentro de Cart
...
#Sincronizar el carrito de compras para procesar el funcionamiento
